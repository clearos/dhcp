...
Patch63:  %{name}-4.1.1-P1-lease-table-overflow.patch
Patch64:  %{name}-4.1.1-P1-ignore-UIDS.patch
Patch65:  %{name}-4.1.1-P1-dhclient-preinit6s.patch
Patch66:  %{name}-4.1.1-P1-client-request-release-bind-iface.patch
...
# or the size exceeds what the code supports. (#1133917)
%patch63 -p1 -b .lease-table-overflow

# Add ignore-client-uids option in the server. This option causes the server to
# not record a client's uid in its lease. This violates the specification but
# may also be useful when a client can dual boot using different client ids
# but the same mac address. (#1196768)
%patch64 -p1 -b .ignore-UIDS

# dhclient: make sure link-local address is ready in stateless mode (#1263466)
%patch65 -p1 -b .preinit6s

# send unicast request/release via correct interface (#1297445)
%patch66 -p1 -b .bind-iface
...
%{__install} -p -m 0644 %{SOURCE10} %{buildroot}%{dhcpconfdir}/dhclient-exit-hooks.d/
...
* Thu Oct 27 2016 Pavel Šimerda <psimerda@redhat.com> - 12:4.1.1-52.P1
- Resolves: #1321945 - dhclient hook script for Azure cloud

* Mon Jan 11 2016 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-51.P1
- send unicast request/release via correct interface (#1297445)

* Thu Dec 03 2015 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-50.P1
